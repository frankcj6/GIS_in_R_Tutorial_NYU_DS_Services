...
In the past few years, the development of new R packages dedicated to geospatial analysis and visualization has contributed to the emergence 
...
* The tutorial is divided into two parts. In the first part, we will cover how to:
  ...
  + Join the tabular data to the shapefile of country boundaries, visualize the tabular data on a map, and export the map for future use in published material.of R as a powerful and user-friendly platform for geospatial analysis. While QGIS was traditionally the software of choice for those looking for an open-source Geographic Information Systems (GIS) option, it is now well worth your time to explore R as a possible open-source solution for your geospatial needs. To that end, this workshop aims to show you how to carry out basic GIS tasks within R Studio.
...
```{r}
#you can view the entire dataset by the following code in #.
#View(trade_gdp)
...
```{r}
# check the structure of shapefile
str(world_shapefile)
```
...
```{r}
#You can read the entire shapefile using the following code in #
#View(world_shapefile)

head(world_shapefile)
```
...
```{r}
# create visualization of the shapefile
...
```{r}
# create visualization of the shapefile with the color of orange
tm_shape(world_shapefile)+
  tm_polygons("orange")
```
...
```{r}
# create visualization of the shapefile with the color of cyan
tm_shape(world_shapefile)+
...
```{r}
# create visualization of the shapefile without the frame
tm_shape(world_shapefile)+
  tm_polygons("lightcyan2")+
...
```{r}
# switch to interactive visualization
tmap_mode("view")
```
...
```{r}
# switch back to static visualization
tmap_mode("plot")
```
...
```{r}
# remove series Name and Series Code from original dataframe
trade_gdp<-select(trade_gdp,-c("Series Name", "Series Code")) %>% 
# rename trade_pct_gdp to 2017 [YR2017]
...
```{r}
# merge world_shapefile and trade_gdp by iso_a3 and country name for non-missing and missing iso_a3 rows respectively
world_trade_shapefile <- rbind(
  left_join(world_shapefile[which(world_shapefile$iso_a3 !="-99"),], trade_gdp[,-which(colnames(trade_gdp) == "Country Name")],by="iso_a3"),
  left_join(world_shapefile[which(world_shapefile$iso_a3 =="-99"),-which(colnames(world_shapefile) == "iso_a3")], trade_gdp, by= c("name" = "Country Name"))
)
```
...
Choropleth Map is a type of thematic map that shows the pattern and distribution of the data by color or patterned propotionately. Some of its most famous use is to display the population density or per-capita income of a country. 
...
```{r}
# turn the data frame to a spatial object by using coordinates
...
head(worldbank_projectlocations_dataset)
```
...
## 11. Turn the data frame of World Bank project locations into a spatial object and plot it using tmap
...
```{r}
# create visualization of the spatial object
project_locations<-
...
```{r}
# create visualization of the spatial object with the color of orange
project_locations<-
  tm_shape(worldbank_locations_spatial)+
  tm_dots("orange")
...
```{r}
# visualize the spatial object in light blue on top of the shapefile in red
worldmap_projects<-
  tm_shape(world_shapefile)+
    tm_polygons("red2")+
  tm_shape(worldbank_locations_spatial)+
...
```{r}
# create a new spatial object by filtering the original spatial object based on the specified conditions
...
```{r}
# visualize the new spatial object in light blue on top of the shapefile in red
...
Reprojection in GIS is actually changing the coordinates value from one coordinate system to another. Simply speaking, it gives you the ability to look at the same data from different views and that is really helpful for someone who wants to see the pattern of the data from a different perspective. 
The main reason to reproject includes facilite the computation of some features while preserve some properties of the original data, changing territory because many reference system are optimized for a specific country. 
...
head(malawi_projects_constituencies_spatialjoin)
```
...
head(projects_per_constituency)
```
## 17. Join our table on projects per constituency created in Step 16 to the initial shapefile of Malawi constituencies based on ID
